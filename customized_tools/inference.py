...
register_coco_instances("isbi_val", {}, "datasets/isbi/annotations/isbi_val.json", "datasets/isbi/isbi_val")
...
#dataset_dicts = DatasetCatalog.get("isbi_val")
dataset_dicts = DatasetCatalog.get("isbi_test")
 
for d in dataset_dicts:  
    im = cv2.imread(d["file_name"])
    #gt
    #visualizer = Visualizer(im[:, :, ::-1], metadata=MetadataCatalog.get("isbi_val"), scale=0.5)
    visualizer = Visualizer(im[:, :, ::-1], metadata=MetadataCatalog.get("isbi_test"), scale=0.5)
    vis = visualizer.draw_dataset_dict(d)
    plt.imsave(os.path.join(outputDir, "results", d["file_name"].split("/")[-1]+'_gt'+'.png'), vis.get_image()[:, :, ::-1])

    #pred
    outputs = predictor(im)
    v = Visualizer(im[:, :, ::-1],
                metadata=MetadataCatalog.get("isbi_test"), 
    )

...
def boundary_loss_kervadec(pred_logits, gt_masks):
    """
    Boundary loss original from Kervadec et al.
    """
    ...
    dist_maps = torch.zeros_like(gt_masks, dtype=torch.float32)
    
    for i in range(gt_masks.shape[0]):
        gt_np = gt_masks[i].cpu().numpy().astype(np.uint8)
        ...
        dist_inside  = distance_transform_edt(gt_np)        
        dist_outside = distance_transform_edt(1 - gt_np)    
        
        phi = dist_outside - dist_inside
        
        
        phi = phi / (np.abs(phi).max() + 1e-8)
        dist_maps[i] = torch.from_numpy(phi).float()
    
    dist_maps = dist_maps.to(pred_logits.device)
    
    
    loss = (dist_maps * pred_probs).mean()
...
def drm_ranking_loss(overlap_logits, nonoverlap_logits,
                     gt_overlap, gt_nonoverlap, margin=0.3):
    
    
    if overlap_logits.dim() == 4:
        overlap_logits    = overlap_logits.squeeze(1)
    if nonoverlap_logits.dim() == 4:
        nonoverlap_logits = nonoverlap_logits.squeeze(1)

    overlap_prob    = torch.sigmoid(overlap_logits)    
    nonoverlap_prob = torch.sigmoid(nonoverlap_logits) 

    
    pure_overlap = (gt_overlap > 0.5) & (gt_nonoverlap < 0.5)
    if pure_overlap.sum() > 0:
        loss_ov = F.relu(
            margin - overlap_prob[pure_overlap]
                   + nonoverlap_prob[pure_overlap]
        ).mean()
    else:
        loss_ov = torch.tensor(0.0, device=overlap_logits.device)

   
    pure_nonoverlap = (gt_nonoverlap > 0.5) & (gt_overlap < 0.5)
    if pure_nonoverlap.sum() > 0:
        loss_nov = F.relu(
            margin - nonoverlap_prob[pure_nonoverlap]
                   + overlap_prob[pure_nonoverlap]
        ).mean()
    else:
        loss_nov = torch.tensor(0.0, device=overlap_logits.device)

    return (loss_ov + loss_nov) / 2  
    
def compute_gt_overlap_nonoverlap(selected_proposals, mask_side_len, device):
    
    gt_overlap_list    = []
    gt_nonoverlap_list = []

    for instances_per_image in selected_proposals:
        if len(instances_per_image) == 0:
            continue

        
        all_masks = instances_per_image.gt_masks.crop_and_resize(
        ...
        for i in range(N):
            ...
            overlap_i = torch.zeros_like(mask_i)
            for j in range(N):
                if i == j:
                    continue
               
                overlap_i = torch.clamp(overlap_i + mask_i * all_masks[j], 0, 1)

            nonoverlap_i = torch.clamp(mask_i - overlap_i, 0, 1)

            gt_overlap_list.append(overlap_i)
            gt_nonoverlap_list.append(nonoverlap_i)

    if not gt_overlap_list:
        return None, None

    gt_overlap_all    = torch.stack(gt_overlap_list,    dim=0) 
    gt_nonoverlap_all = torch.stack(gt_nonoverlap_list, dim=0)  

    return gt_overlap_all, gt_nonoverlap_all
...
class ROIHeads(torch.nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.use_drm_se = True
        drm_channels=3* cfg.MODEL.ROI_TRIPLE_BRANCH_WHOLE_MASK_HEAD.CONV_DIM
        reduction=16
        self.drm_se_conv1 = nn.Conv2d(drm_channels, drm_channels // reduction, 1)
        self.drm_se_conv2 = nn.Conv2d(drm_channels // reduction, drm_channels, 1)
        #
    ...
    def forward_with_given_boxes(
        ...
        if self.refinement:
            if self.pure_branch:
                ...
                if self.use_drm_se:
                    se = F.adaptive_avg_pool2d(concatenated_mask_feature, 1)
                    se = F.relu(self.drm_se_conv1(se))
                    se = torch.sigmoid(self.drm_se_conv2(se))
                    concatenated_mask_feature = concatenated_mask_feature * se + concatenated_mask_feature
        ...
        if self.training:
            ...
            if self.refinement:
                if self.pure_branch:
                    ...
                    concatenated_mask_feature = torch.cat([mask_features]+[triple_branch_overlapping_mask_inner_feature]+[triple_branch_nonoverlapping_mask_inner_feature], 1)
            ...
            if self.consis_loss:
                ...
                """
                    ...
                    #xor soft
                    whole = torch.sigmoid(triple_branch_whole_mask_logits)
                    overlap = torch.sigmoid(triple_branch_overlapping_mask_logits)
                    nonoverlap= torch.sigmoid(triple_branch_nonoverlapping_mask_logits)
                    soft_xor= overlap*(1-nonoverlap)+nonoverlap*(1-overlap)
                    soft_xor=soft_xor.detach()
                    cons_loss=F.binary_cross_entropy(whole, soft_xor)
                    triple_branch_consistency_loss={"loss_triple_branch_consistency": 0.01 * cons_loss}
                 
                
                    #ranking loss
                    mask_side_len = triple_branch_overlapping_mask_logits.shape[-1]

                    gt_overlap_all, gt_nonoverlap_all = compute_gt_overlap_nonoverlap(
                        selected_proposals,
                        mask_side_len=mask_side_len,
                        device=triple_branch_overlapping_mask_logits.device,
                       )
                       
                     
                    if gt_overlap_all is not None:
                        assert gt_overlap_all.shape[0] == triple_branch_overlapping_mask_logits.shape[0], \
                        f"Mismatch: gt={gt_overlap_all.shape[0]} vs logits={triple_branch_overlapping_mask_logits.shape[0]}"
                    if gt_overlap_all is not None:
                        loss_ranking = drm_ranking_loss(
                        triple_branch_overlapping_mask_logits,
                        triple_branch_nonoverlapping_mask_logits,
                        gt_overlap_all,
                        gt_nonoverlap_all,
                        margin=0.3,
                        )
                        triple_branch_consistency_loss["loss_drm_ranking"] = 0.1 * loss_ranking
                ...
                if self.use_boundary_loss:
                    mask_side_len = triple_branch_whole_mask_logits.shape[-1]  
                    gt_whole_list = []
                    for instances_per_image in selected_proposals:
                        if len(instances_per_image) == 0:
                            continue
                        gm = instances_per_image.gt_masks.crop_and_resize(
                        instances_per_image.proposal_boxes.tensor,
                        mask_side_len
                        ).to(device=triple_branch_whole_mask_logits.device).float()
                        gt_whole_list.append(gm)   

                    if gt_whole_list:
                        gt_whole_all = cat(gt_whole_list, dim=0)   
                        
                        loss_b = boundary_loss_kervadec(
                        triple_branch_whole_mask_logits,gt_whole_all)
                        
                    losses["loss_boundary"] = 0.1 * loss_b
